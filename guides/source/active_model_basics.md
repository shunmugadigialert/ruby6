...
`ActiveModel::AttributeAssignment` adds support assigning multiple attributes all at once with the `#assign_attributes` method. the module also provides an `#attributes=` alias for convenience.
...
person = Person.new
person.assign_attributes first_name: "Ruby", last_name: "Novice"
person.attributes = { last_name: "Expert" }
```

Attribute assignment also supports multi-parameter values like the ones submitted by Action View's [form helpers for Date and Time values](/form_helpers.html#using-date-and-time-form-helpers). For example, imagine a Date field generated by [date_select](https://api.rubyonrails.org/classes/ActionView/Helpers/DateHelper.html#method-i-date_select):

```erb
<%= form.date_select :born_on %>
```

When the form with that field is submitted to the server, Action Pack will parse its the parameters into a Hash, which the class accepts as an attribute value. For example, if the fields selected November 24, 2004, the attribute assignment would resemble something like:

```ruby
person.born_on = { "born_on(1i)" => "2004", "born_on(2i)" => "11", "born_on(3i)" => "24" }
```

That assignment will call the `#born_on` method with the value split apart even further:

```ruby
def born_on=(value)
  value # => { 1 => 2004, 2 => 11, 3 => 24 }

  Date.new(value[1], value[2], value[3])
end
```

If the class also includes `ActiveModel::Attributes`, it can define `born_on` as a `:date` attribute to automatically support translating multi-parameter values into `Date` instances:
...
The same built-in support is provided for transforming `:datetime` values into
`DateTime` instances, and `:time` values into `Time` instances.
