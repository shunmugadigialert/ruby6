...
module ActionView
  module Helpers # :nodoc:
    ...
    module TagHelper
      ...
      #
      # Data attribute values are encoded to JSON, with the exception of +String+, +Symbol+, and
      # +BigDecimal+ instances.
      # This may come in handy when using the built-in +HTMLElement.dataset+
      # property or jQuery's HTML5-aware +.data()+ from 1.4.3.
      #
      ...
      class TagBuilder
        ...
        def self.define_element(name, code_generator:, method_name: name.to_s.underscore) # :nodoc:
        ...
        def self.define_void_element(name, code_generator:, method_name: name.to_s.underscore) # :nodoc:
          code_generator.define_cached_method(method_name, namespace: :tag_builder) do |batch|
            batch.push(<<~RUBY)
              def #{method_name}(content = nil, escape: true, **options, &block)
                if content || block
        ...
        def self.define_self_closing_element(name, code_generator:, method_name: name.to_s.underscore) # :nodoc:
          code_generator.define_cached_method(method_name, namespace: :tag_builder) do |batch|
            batch.push(<<~RUBY)
              def #{method_name}(content = nil, escape: true, **options, &block)
                if content || block
      ...
      # === Modern syntax
      #
      # When invoked without any arguments, options, or a block, +tag+ will
      # return an instance of ActionView::Helpers::TagHelper::TagBuilder.
      #
      # Builds HTML5 compliant tags with a tag proxy. Every tag can be built with:
      #
      #   tag.<tag name>(optional content, options)
      ...
      private
        def ensure_valid_html5_tag_name(name) # :nodoc:
        ...
        def build_tag_values(*args) # :nodoc:
          tag_values = []
